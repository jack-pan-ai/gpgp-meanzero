...
#' ## not run
#' # fit <- fit_model(y, locs, "matern_isotropic")
#' # fit
#'
#'
#' @export
fit_model_meanzero <- function(y, locs, covfun_name = "matern_isotropic",
    ...
    not_missing <- apply( cbind(y,locs), 1,
        function(x){
    ...
    get_start_parms <- function(y, locs){
      start_var <- var(y)
      start_smooth <- 0.8
      start_nug <- 0.1
      n <- length(y)
    ...
    get_linkfun <- function(covfun_name){
      ...
      return(list(link = link, dlink = dlink, invlink = invlink, 
    ...
    get_penalty <- function (y, locs, covfun_name){
      ...
      pen_nug <- function(x, j) {
        pen_loglo(x[j], 0.1, log(0.001))
      }
      dpen_nug <- function(x, j) {
        dpen <- rep(0, length(x))
        dpen[j] <- dpen_loglo(x[j], 0.1, log(0.001))
        return(dpen)
      }
      ddpen_nug <- function(x, j) {
        ddpen <- matrix(0, length(x), length(x))
        ddpen[j, j] <- ddpen_loglo(x[j], 0.1, log(0.001))
        return(ddpen)
      }
      pen_sm <- function(x, j) {
        pen_loglo(x[j], 0.1, log(0.2))
      }
      dpen_sm <- function(x, j) {
        dpen <- rep(0, length(x))
        dpen[j] <- dpen_loglo(x[j], 0.1, log(0.2))
        return(dpen)
      }
      ddpen_sm <- function(x, j) {
        ddpen <- matrix(0, length(x), length(x))
        ddpen[j, j] <- ddpen_loglo(x[j], 0.1, log(0.2))
        return(ddpen)
      }
      pen_var <- function(x, j) {
        pen_hi(x[j]/vv, 1, 6)
      }
      ...
      if (covfun_name == "matern_isotropic"){
        ...
        dpen <- function(x){
          dpen_nug(x, 4) + dpen_sm(x, 3) + dpen_var(x, 1) + 
            dpen_sm_hi(x, 3)
        }
        ddpen <- function(x){
          ddpen_nug(x, 4) + ddpen_sm(x, 3) + ddpen_var(x, 1) + 
            ddpen_sm_hi(x, 3)
        }
      }
      return(list(pen = pen, dpen = dpen, ddpen = ddpen))}

    penalty <- get_penalty(y,locs,covfun_name) 
    pen <- penalty$pen
    dpen <- penalty$dpen
    ddpen <- penalty$ddpen
    ...
    for(i in 1:length(m_seq)){
        ...
        if(group){
            ...
            likfun <- function(logparms){
                ...
                likobj <- vecchia_grouped_meanzero_loglik_grad_info(
                    link(lp),covfun_name,yord,locsord,NNlist)
            ...
            likfun <- function(logparms){
                ...
                likobj <- vecchia_meanzero_loglik_grad_info(
                    link(lp),covfun_name,yord,locsord,NNarray[,1:(m+1)])
                likobj$loglik <- -likobj$loglik - pen(link(lp))
                likobj$grad <- -c(likobj$grad)*dlink(lp) -
                    dpen(link(lp))*dlink(lp)
                likobj$info <- likobj$info*outer(dlink(lp),dlink(lp)) -
        ...
        fit <- fisher_scoring_meanzero( likfun,invlink(start_parms)[active],
