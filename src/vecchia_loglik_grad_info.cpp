...
//' NNarray <- find_ordered_nn(locs,20)
//' NNlist <- group_obs(NNarray)
//' #loglik <- vecchia_grouped_meanzero_loglik_grad_info( 
//' #    covparms, "matern_isotropic", y, locs, NNlist )
//' @export
// [[Rcpp::export]]
List vecchia_grouped_meanzero_loglik_grad_info( 
    NumericVector covparms, 
    StringVector covfun_name,
    NumericVector y,
    const NumericMatrix locs,
...
//' ord <- order_maxmin(locs)
//' NNarray <- find_ordered_nn(locs,20)
//' #loglik <- vecchia_meanzero_loglik_grad_info( 
//' #    covparms, "matern_isotropic", y, locs, NNarray )
//' @export
// [[Rcpp::export]]
List vecchia_meanzero_loglik_grad_info( 
    ...
    // maybe the synthesize functions should take in an argument that
    // says which compute_pieces function to use
    synthesize_grouped(covparms, covfun_name, locs, NNarray, y, X,
        &ll, &betahat, &grad, &info, &betainfo, false, true 
    );
